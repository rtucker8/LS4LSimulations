...
####################### DEFINE MODEL #######################
def define_model_simple(dataframe, batch_id, target_ids):
    ...
    if no_variability_in_outcome == True:
        ...
        # lvl_eng=[]
        # for engagement in lvl_engagement:
        #     if engagement in engagement_values:
        #         lvl_eng.append(engagement)
...
#No longer including interaction_terms in the simple algorithm model- Meeting with Walter, 10-11-24
#We are okay with a simple misspecified model
...
#         if len(dataframe['S1'].unique()) > 1:       
#             for var in interaction_terms:
#                model_str += ' + ' + var + ':C(S1)'
               
#         if len(dataframe['S2'].unique()) > 1:       
#             for var in interaction_terms:
#                model_str += ' + ' + var + ':C(S2)'
...
def run_alg_simple(raw_data_filename, batch_id, target_ids):
    ...
    try:
        model_formula = define_model_simple(dataframe, batch_id, target_ids)
    except Exception as e:
        ...
        try:
            ...
            print("Simple model could not be fit to the observed data from this batch. Megatables were not updated, will continue to use last available megatable.")
        else: 
...
def main(): 
    ...
    # batch_data['engagement']=batch_data.engagement.astype('str')
        ...
        if batch != 14:
            save_raw_data = 'alg_output_simple_extreme2/' + str(sys.argv[1]) + '_data_raw_batch' + str(update_id) + '_g' + str(g) + '.csv'
            data_so_far.to_csv(save_raw_data, index=False)
        elif batch == 14:
            save_raw_data = 'alg_output_simple_extreme2/results_simple_extreme2/' + str(sys.argv[1]) + '_data_raw_batch' + str(update_id) + '_g' + str(g) + '.csv'
            data_so_far.to_csv(save_raw_data, index=False)
            break

        # Step 4: Run simple algorithm (process data + fit model + generate megatable)
        alg='simple'
        result=run_alg_simple(save_raw_data, update_id, target_ids)
        
        if not (result is None):
            # read in current mega table
            mega_tabs = pd.read_csv('alg_output_simple_extreme2/' + str(sys.argv[1]) + '_tx_prob_table_small_batch' + str(update_id) + '_g' + str(g) + '.csv')
