...
def define_model_complicated(dataframe, batch_id, target_ids):
    ...
    if no_variability_in_outcome == True:
        ...
        print("Values of Columns in Dataframe:")
        for col in cols:
            print(str("Column Name:") + str(col))
            print(dataframe[col].unique())
        # now define model in bambi function 
        # note: to consider only complete data, use , dropna=True     

        logging.info("About to run bm.Model()") 
        print("About to run bm.Model")
        model_formula = bm.Model(model_str, dataframe, family = "bernoulli", priors = pr, dropna=True)
        logging.info("Finished running bm.Model()")
        print("Finished running bm.Model")
...
def main(): 
    ...
    # batch_data['engagement']=batch_data.engagement.astype('str')
        ...
        if batch != 0:
            ...
            prev_raw_data = pd.read_csv('alg_output_complicated_extreme1/' + str(sys.argv[1]) + '_data_raw_batch' + str(prev_update_id) + '_g' + str(g) + '.csv')
        ...
        if batch != 14:
            save_raw_data = 'alg_output_complicated_extreme1/' + str(sys.argv[1]) + '_data_raw_batch' + str(update_id) + '_g' + str(g) + '.csv'
            data_so_far.to_csv(save_raw_data, index=False)
        elif batch == 14:
            save_raw_data = 'alg_output_complicated_extreme1/results_complicated_extreme1/' + str(sys.argv[1]) + '_data_raw_batch' + str(update_id) + '_g' + str(g) + '.csv'
            data_so_far.to_csv(save_raw_data, index=False)
            break
