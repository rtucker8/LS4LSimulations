...
def define_model(dataframe, batch_id, target_ids):
    ...
    save_messages_name = 'alg_output_' + str(alg) +'_' + str(mech) + '/' + str(sys.argv[1]) + 'model_messages_batch' + '_g' + str(g) + '.txt' # MODIFIED
...
def main(): 
    for batch in range(update_dates.shape[0]): #index from 0 to 13 corresponds to rows in update_dates
       ...
       # batch_data['engagement']=batch_data.engagement.astype('str')
        ...
        if batch != 14:
            save_raw_data = 'alg_output_LS4L_extreme2/' + str(sys.argv[1]) + '_data_raw_batch' + str(update_id) + '_g' + str(g) + '.csv'
            data_so_far.to_csv(save_raw_data, index=False)
        elif batch == 14:
            save_raw_data = 'alg_output_LS4L_extreme2/results_LS4L_extreme2/' + str(sys.argv[1]) + '_data_raw_batch' + str(update_id) + '_g' + str(g) + '.csv'
            data_so_far.to_csv(save_raw_data, index=False)
            break
        ...
        mega_tabs = pd.read_csv('alg_output_LS4L_extreme2/' + str(sys.argv[1]) + '_tx_prob_table_small_batch' + str(update_id) + '_g' + str(g) + '.csv')
