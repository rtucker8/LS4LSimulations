...
def main(): 
    ...
    # batch_data['engagement']=batch_data.engagement.astype('str')
        ...
        if batch != 14:
            save_raw_data = 'alg_output_simple_extreme1/' + str(sys.argv[1]) + '_data_raw_batch' + str(update_id) + '_g' + str(g) + '.csv'
            data_so_far.to_csv(save_raw_data, index=False)
        elif batch == 14:
            save_raw_data = 'alg_output_simple_extreme1/results_simple_extreme1/' + str(sys.argv[1]) + '_data_raw_batch' + str(update_id) + '_g' + str(g) + '.csv'
            data_so_far.to_csv(save_raw_data, index=False)
            break
        ...
        if not (result is None):
            ...
            mega_tabs = pd.read_csv('alg_output_simple_extreme1/' + str(sys.argv[1]) + '_tx_prob_table_small_batch' + str(update_id) + '_g' + str(g) + '.csv')
